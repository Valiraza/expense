...
import { LayoutDashboard, TrendingDown, TrendingUp, Wallet, DollarSign } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Solde Actuel" value={`€ ${(data.balance ?? 0).toFixed(2)}`} color="text-indigo-600" icon={<Wallet />} />
        <StatCard title="Total Revenus" value={`€ ${(data.totalIncomes ?? 0).toFixed(2)}`} color="text-green-600" icon={<TrendingUp />} />
        <StatCard title="Total Dépenses" value={`€ ${(data.totalExpenses ?? 0).toFixed(2)}`} color="text-red-600" icon={<TrendingDown />} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={data.categoryStats} dataKey="amount" nameKey="category" cx="50%" cy="50%" outerRadius={80} label>
